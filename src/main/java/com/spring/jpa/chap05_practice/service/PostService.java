...
import com.spring.jpa.chap05_practice.repository.HashTageRepository;
...
public class PostService {
  ...
  public PostDetailResponseDTO insert(final PostCreateDTO dto)
    ...
    if(hashTags != null && hashTags.size() > 0) {
      hashTags.forEach(ht -> {
        HashTag savedTag = hashTageRepository.save(
                HashTag.builder()
                        .tagName(ht)
                        .post(saved)
                        .build()
      ...
      //추후에 진행되는 관정에서 문제가 발생하지 않습니다.(연관관계의 주인이 아니기 때문)
