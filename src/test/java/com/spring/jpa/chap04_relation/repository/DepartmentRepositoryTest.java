...
class DepartmentRepositoryTest {
  ...
  @Test
  @DisplayName("특정 부서를 조회하면 해당 부서원들도 함께 조호되어야 한다.")
  void testFIndDept() {
      //given
      long id =2L;
      //when
  ...
  void testChangeDept() {
    ...
    //사원의 부서정보를 업데이트 하면서, 부서에 대한 정보도 같이 업데이트.(양방향 2번째)
    foundEmp.setDepartment(newDept); //department에서도 조회가 가능하다.

    employeeRepository.save(foundEmp);

    //변경 감지(더티 체크) 후 변경된 내용을 DB에 반영하는 역할을 합니다. :강제 (양방향 1번째)
//    entityManager.flush();
//    entityManager.clear();

    //then
    //1번 부서 정보를 조회해서 모든 사원을 보겠다. (3명의 사원이 올것이다.)
